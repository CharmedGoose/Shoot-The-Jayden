...
public static class MeshGenerator
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve _heightCurve, int levelOfDetail, bool useFlatShading) 
    {
        ...
        int numVertsPerLine = MapGenerator.MapChunkSize;

        Vector2 topLeft = new Vector2(-1, 1) * (numVertsPerLine - 1) / 2f;

        MeshData meshData = new(numVertsPerLine, skipIncrement, useFlatShading);
        ...
        for (int y = 0; y < numVertsPerLine; y++)
        {
            for (int x = 0; x < numVertsPerLine; x++)
            {
                ...
                if (!isSkippedVertex) {
                    ...
                    Vector2 vertexPosition2D = topLeft + new Vector2(percent.x, -percent.y) * (numVertsPerLine - 1);
...
public class MeshData
{
    ...
    Vector3[] CaclulateNormals()
    {
        ...
        borderTriangleCount = outOfMeshTriangles.Length / 3;

        for (int i = 0; i < borderTriangleCount; i++)
        {
            normalTriangleIndex = i * 3;
            vertexIndexA = outOfMeshTriangles[normalTriangleIndex];
            vertexIndexB = outOfMeshTriangles[normalTriangleIndex + 1];
            vertexIndexC = outOfMeshTriangles[normalTriangleIndex + 2];
    ...
    {
        pointA = (indexA < 0) ? outOfMeshVertices[-indexA-1] : vertices[indexA];
        pointB = (indexB < 0) ? outOfMeshVertices[-indexB - 1] : vertices[indexB];
        pointC = (indexC < 0) ? outOfMeshVertices[-indexC - 1] : vertices[indexC];
